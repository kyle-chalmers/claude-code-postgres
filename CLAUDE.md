...
- Read-only (everything else): connect as `analyst_ro`. This is the login to use for all analysis. Local: `psql -h localhost -p 55432 -U analyst_ro -d postgres`. Cloud: `psql "postgresql://analyst_ro@<endpoint>.<region>.aws.neon.tech/<db>?sslmode=require"`.
