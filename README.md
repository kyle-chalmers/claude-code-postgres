...
If you do want the structured-tooling path, set it up the way the vendor recommends. Supabase, for example, says to use their MCP server against a development project, or in read-only and project-scoped mode if real data is unavoidable: `claude mcp add supabase --env SUPABASE_ACCESS_TOKEN=<token> -- npx -y @supabase/mcp-server-supabase@latest --read-only --project-ref=<ref> --features=database,docs`. They also ship [Postgres best-practices Agent Skills](https://github.com/supabase/agent-skills) so the agent writes better SQL (`claude plugin marketplace add supabase/agent-skills`, then `claude plugin install postgres-best-practices@supabase-agent-skills`). One caveat worth knowing: that read-only mode blocks writes but connects as a managed read-only user, not a role scoped to your curated schema, so it carries the write guardrail only. The curated, PII-safe guardrail still depends on connecting as the scoped read-only role below, which is exactly what plain `psql` lets you do. (The `--env` token is also written to your MCP config; Supabase's hosted OAuth connection keeps no token on disk.)
